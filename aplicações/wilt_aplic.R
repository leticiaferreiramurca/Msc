...
for (i in 1:length(modelos)){
  ...
  parametros[i, "beta4"] = mean(extract(modelos[[i]], permuted = TRUE)$beta[,4])
  if(i %in% c(2,3)){
...
ggsave(path = '/home/notmyname/Desktop/mestrado/imagens', filename = "histres2.png", width = 6, height = 4)

data = data.frame("res" = res, "index" = 1:length(res))
ggplot(data = data, aes(x = index, y = res)) +
  geom_point(size = 2) +
  labs(x = "Index", y = "Resíduos Quantílicos")+theme_bw()
ggsave(path = '/home/notmyname/Desktop/mestrado/imagens', filename = "res2.png", width = 6, height = 4)
